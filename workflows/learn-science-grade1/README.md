...
every other grade, about 3003 characters in total. Every experiment
...
- `tests/giztest/smoke/learn-science-grade1.<implementation>.giztest.yaml`: speed, latency and responsiveness.
- `tests/giztest/quality/learn-science-grade1.<implementation>.giztest.yaml`: quality control and safety guardrails.
- `tests/giztest/soak/learn-science-grade1.<implementation>.giztest.yaml`: long-turn Tester relay with reload.

Run `make test-e2e TIER=smoke RAID=learn-science-grade1`. Each tier has one file per implementation, run concurrently with `PARALLEL=4`; see [the test guide](../../tests/giztest/README.md) for budgets and failure reporting.
