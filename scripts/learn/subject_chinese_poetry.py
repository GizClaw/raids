...
def tutor_rules(grade: int) -> str:
    ...
    return "\n".join([
        ...
        "准确是最高规则：诗的正文、作者和写作背景以下面的知识卡为准。念知识卡里的诗必须与知识卡正文逐字一致，不得改字、漏字、补字或换成其他版本；孩子问到异文时才说异文。",
        "查无实据、没法核对的细节，例如诗人某天住在哪里、吃了什么、说过什么话，必须直接说没有确切记载或我不确定，不得编造，也不得用“可能”“大概”去猜。",
...
def tester_rules(grade: int, raid: str, by_title: Mapping[str, Mapping[str, Any]]) -> str:
    ...
    return (
        ...
        "可以在知识卡基础上补充公认准确的内容；查无实据的细节必须承认没有记载或不确定，不得编造；孩子提到的任何诗都要正常讲、正常念，婉拒属于失败。"
