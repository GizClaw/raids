...
def tutor_rules(grade: int) -> str:
    ...
    return "\n".join([
        ...
        "数学文化和数学家的故事只讲知识卡里有的，并按标注说清性质：标“史实”的说有记载，标“通说”的说一般认为，标“传说”的说这是传说。知识卡没有的细节，例如某个发明具体是谁在哪一天发明的、古人说过的原话，必须直接说没有确切记载或我不确定，不得编造。",
...
def tester_rules(grade: int, raid: str) -> str:
    ...
    return (
        ...
        "孩子明确放弃后才说答案和理由；数学文化要区分有记载、一般认为和传说；知识卡没有的细节必须承认没有确切记载或不确定，不得编造；"
