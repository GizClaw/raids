...
- Cast: narrator (visible facts and transitions), 女主角 (`heroine`), and 伙伴 (`companion`); each character keeps a distinct motive, voice, and knowledge boundary.
...
| --- | --- | --- | --- | --- | --- |
| 1. 善意之门 | first opening request | resolve this chapter's core tension | observe a concrete consequence; compare the two characters' responses | player resolves the current hook and requests chapter 2 | emit the chapter 2 heading once, followed directly by that chapter's opening scene |
...
| 4. 温暖归途 | chapter 3 transition satisfied and player explicitly continues | resolve this chapter's core tension | observe a concrete consequence; compare the two characters' responses | player confirms a durable choice and one remaining responsibility | remain in chapter 4; keep one post-ending responsibility |
...
- `flowcraft.roles.giztest.yaml` creates isolated narrator/heroine/companion Workspaces and requires text EOS, audio EOS, non-empty Opus, timing evidence, and direct, speaker-label-free role text.
...
GIZCLAW=/absolute/path/to/gizclaw-v0.7.7 GIZCLAW_TEST_CLI=/absolute/path/to/gizclaw-v0.7.7 GIZCLAW_CONTEXT=e2e-server-volc-bj-01 GIZCLAW_TEST_ENDPOINT=edge-bj-01.e2e.gizclaw.com:9821 GIZCLAW_TEST_REGISTRATION_TOKEN=<testing-runtime-token> APPLY=1 RAID=story-fairy-tales PARALLEL=3 make test-e2e
```
...
- `eino.multi-role.yaml`: `eino-story-fairy-tales-multi-role`; Voice aliases: `eino-story-fairy-tales-mr.storyteller`, `eino-story-fairy-tales-mr.heroine`, `eino-story-fairy-tales-mr.companion`, `eino-story-fairy-tales-mr.fairy`, `eino-story-fairy-tales-mr.elder`.
- `flowcraft.multi-role.yaml`: `flowcraft-story-fairy-tales-multi-role`; Voice aliases: `flowcraft-story-fairy-tales-mr.storyteller`, `flowcraft-story-fairy-tales-mr.heroine`, `flowcraft-story-fairy-tales-mr.companion`, `flowcraft-story-fairy-tales-mr.fairy`, `flowcraft-story-fairy-tales-mr.elder`.
