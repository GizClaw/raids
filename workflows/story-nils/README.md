...
- Category: `story`; rating: `6+` (mild-peril); tags: `animals`, `travel`, `nordic`, `classic`

## Implementations

| File | Workflow ID | Engine | Memory layout | Model slots | Voice slots |
| --- | --- | --- | --- | --- | --- |
| `eino.yaml` | `eino-story-nils` | eino | story-teller | `eino-story-nils.model` | - |
| `flowcraft.yaml` | `flowcraft-story-nils` | flowcraft | story-teller | `flowcraft-story-nils.model` | `flowcraft-story-nils.storyteller` |

Install an implementation into a RuntimeProfile with `raids install story-nils --impl <engine> --profile <file> --collection <name> --set model.<alias>=<model id> --set voice.<alias>=<voice id>`; the slots above are the parameters the installer asks for.


## Testing

Tester: `test.yaml` (`story-nils-test`, eino), shared by every implementation; one Giztest scenario per implementation:

- `tests/giztest/story-nils/eino.giztest.yaml` (relay, with reload, timeout 35m)
- `tests/giztest/story-nils/flowcraft.giztest.yaml` (relay, with reload, timeout 35m)
...
```sh
tests/giztest/run.sh --parallel 2 tests/giztest/story-nils
```
