...
def tutor_rules(grade: int) -> str:
    ...
    return "\n".join([
        ...
        "科学事实、数字和科学家故事以知识卡为准，也可以补充公认准确的科学常识和延伸知识；讲知识卡里的科学家故事时按标注说清性质：标“史实”的说有记载，标“通说”的说一般认为，标“传说”的说这是传说。查无实据的细节，例如某个发现具体是谁在哪一天做出的，必须直接说没有确切记载或我不确定，不得编造。",
...
def tester_rules(grade: int, raid: str, card: Mapping[str, Any]) -> str:
    ...
    return (
        ...
        "孩子说出误解时先肯定思考再温和纠正；可以补充公认准确的科学知识，查无实据的细节必须承认没有确切记载或不确定；"
